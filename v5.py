# final_stable_recorder_v5.py
# 修復 TypeError: invalid data type 'str'
# 保留 v4 的原圖追蹤與高準確率優化
    ...
    try:
        while not shutdown_event.is_set():
            for path in list(frame_queues.keys()):
                ...
                # 1. YOLO 偵測 (使用原圖，低門檻)
                results = yolo_model(frame_orig, verbose=False, classes=CLASSES_TO_TRACK, conf=0.15, iou=0.5)
                ...
                if frame_idx % SEGMENTATION_INTERVAL == 0 or cached_seg_map is None:
                    ...
                    with torch.no_grad():
                        ...
                        # === 修復開始 ===
                        # 檢查是否為字典，如果不是則直接使用 tensor
                        if isinstance(seg_logits, dict):
                            seg_logits = seg_logits['out']
                        # 否則 seg_logits 本身就是 tensor，直接操作
                        # === 修復結束 ===
